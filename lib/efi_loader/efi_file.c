...
{
	return fs_set_blk_dev2(fh->desc, fh->part);
}
...
static efi_status_t EFIAPI efi_file_device_path_open(void *handle,
		efi_guid_t *protocol, void **protocol_interface)
{
	struct file_handle *fh = handle;

	if (!fh->dp) {
		fh->dp = efi_dp_from_file(fh->desc, fh->part, fh->path);
	}

	*protocol_interface = fh->dp;
 ...
 */
static struct efi_file_handle *file_open(struct blk_desc *desc, int part,
		struct file_handle *parent, s16 *file_name)
{
	...
	/* +2 is for null and '/' */
	fh = calloc(1, sizeof(*fh) + plen + flen + 2);
	...
	fh->efiobj.handle = fh;
	fh->efiobj.protocols[0].guid = &efi_guid_device_path;
	fh->efiobj.protocols[0].open = efi_file_device_path_open;

	/* Hook up to the object list */
	list_add_tail(&fh->efiobj.link, &efi_obj_list);
		...
		s16 *file_name, u64 open_mode, u64 attributes)
{
	...
	*new_handle = file_open(fh->desc, fh->part, fh, file_name);
	if (!*new_handle)
...
{
	struct simple_file_system *fs = to_fs(this);

	EFI_ENTRY("%p, %p", this, root);

	*root = file_open(fs->desc, fs->part, NULL, NULL);
